...
import com.example.mvvmovie.ui.MovieAdapter
import com.example.mvvmovie.utils.DiffUtils
import com.example.mvvmovie.utils.setImageUrl

class PopularMovieAdapter (private val clickListener: OnItemPopClickListener): RecyclerView.Adapter<PopularMovieAdapter.ViewHolder>() {
    private var movieList = ArrayList<Result>()

    class ViewHolder private constructor(private val binding: PopularRowLayoutBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(item: Result, clickListener: OnItemPopClickListener, position: Int) {
            binding.apply {
...
class OnItemPopClickListener(val clickListener: (data: Result, pos: Int) -> Unit) {
